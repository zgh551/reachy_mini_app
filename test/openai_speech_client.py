...
def decode_and_resample(audio_bytes, target_sr=16000):
...
def run_tts_generation(args) -> None:
    ...
    audio_np, sr = decode_and_resample(response.content, target_sr=16000)
    with ReachyMini(media_backend="default") as mini:
        mini.media.start_playing()
        mini.media.push_audio_sample(audio_np)
        time.sleep(audio_np.shape[0] / mini.media.get_output_audio_samplerate())
        mini.media.stop_playing()
    # Save audio response
    output_path = args.output or "tts_output.wav"
    print(type(response.content))
    output_path = "resampled_16k.wav"
    sf.write(output_path, audio_np, sr, subtype='FLOAT')
